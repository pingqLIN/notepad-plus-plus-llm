...
void Notepad_plus::macroPlayback(Macro macro,
                                 std::vector<Document> *pDocs4EndUAIn) {
  ...
  for (Macro::iterator step = macro.begin(); step != macro.end(); ++step) {
    ...
    if (curSciDoc != prevSciDoc) {
      ...
      if (std::find(pDocs4EndUA->begin(), pDocs4EndUA->end(), curSciDoc) ==
          pDocs4EndUA->end()) {
        // not in the list of the macro affected docs so far
        _pEditView->execute(
            SCI_BEGINUNDOACTION); // the macro step will touch another doc, open
                                  // another undo action
        pDocs4EndUA->push_back(
            curSciDoc); // store for possible ending undo action later
      }
    ...
    if (step->isScintillaMacro())
      ...
      _findReplaceDlg.execSavedCommand(
          step->_message, step->_lParameter,
          string2wstring(step->_sParameter, CP_UTF8));
  }

  if (!pDocs4EndUAIn) {
    // handle all the affected docs undo actions closing (for local-only doc
    // list)

    Document invisSciDoc = _invisibleEditView.execute(
        SCI_GETDOCPOINTER); // store the view's original doc
    while (!pDocs4EndUA->empty()) {
      Document doc = pDocs4EndUA->back();
      if (MainFileManager.getBufferFromDocument(doc) == BUFFER_INVALID) {
        // affected doc no longer exists (a macro step closed its associated
        // Notepad++ tab/buffer), the ending undo action is not needed (until
        // Notepad++ supports tab/buffer closing undo)
      } else {
...
void Notepad_plus::command(int id) {
  ...
  case IDM_EDIT_INSERT_DATETIME_LONG: {
    ...
    int dateFlag =
        (id == IDM_EDIT_INSERT_DATETIME_SHORT) ? DATE_SHORTDATE : DATE_LONGDATE;
    GetDateFormatEx(LOCALE_NAME_USER_DEFAULT, dateFlag, &currentTime, NULL,
                    dateStr, sizeof(dateStr) / sizeof(dateStr[0]), NULL);
    GetTimeFormatEx(LOCALE_NAME_USER_DEFAULT, TIME_NOSECONDS, &currentTime,
                    NULL, timeStr, sizeof(timeStr) / sizeof(timeStr[0]));
  ...
  case IDM_EDIT_INSERT_DATETIME_CUSTOMIZED: {
    ...
    wstring dateTimeStr =
        getDateTimeStrFrom(nppGUI._dateTimeFormat, currentTime);
  ...
  case IDM_FILE_OPEN_FOLDER: {
    ...
    if (com.isInitialized()) {
      ...
      hr = ::SHParseDisplayName(
          _pEditView->getCurrentBuffer()->getFullPathName(), nullptr, &pidl, 0,
          nullptr);
      if (SUCCEEDED(hr)) {
    ...
    if (hr == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND)) {
      ...
      // - either the COM cannot be used or the above shell APIs mysteriously
      // fail on some systems
      //   with the "file not found" even though the file is there
      // - do not use this fallback for any other possible error (like
      // E_INVALIDARG, etc.)
      ::ShellExecuteW(_pPublicInterface->getHSelf(), L"explore",
                      std::filesystem::path(
                          _pEditView->getCurrentBuffer()->getFullPathName())
                          .parent_path()
                          .c_str(),
  ...
  case IDM_FILE_CONTAININGFOLDERASWORKSPACE: {
    ...
    ::SendMessage(_pPublicInterface->getHSelf(), NPPM_GETFULLCURRENTPATH,
                  CURRENTWORD_MAXLENGTH,
                  reinterpret_cast<LPARAM>(currentFile.get()));
    ::SendMessage(_pPublicInterface->getHSelf(), NPPM_GETCURRENTDIRECTORY,
                  CURRENTWORD_MAXLENGTH,
                  reinterpret_cast<LPARAM>(currentDir.get()));
  ...
  case IDM_FILE_OPEN_DEFAULT_VIEWER: {
    ...
    HINSTANCE res = ::ShellExecute(NULL, L"open", buf->getFullPathName(), NULL,
                                   NULL, SW_SHOW);

    // As per MSDN
    // (https://msdn.microsoft.com/en-us/library/windows/desktop/bb762153(v=vs.85).aspx)
    // If the function succeeds, it returns a value greater than 32.
    // If the function fails, it returns an error value that indicates the cause
    // of the failure.
    ...
    if (retResult <= 32) {
      ...
      ::MessageBox(_pPublicInterface->getHSelf(), errorMsg.c_str(),
                   L"ShellExecute - ERROR", MB_ICONINFORMATION | MB_APPLMODAL);
    }
  } break;

  case IDM_FILE_OPENFOLDERASWORKSPACE: {
    const NativeLangSpeaker *pNativeSpeaker =
        NppParameters::getInstance().getNativeLangSpeaker();
    wstring openWorkspaceStr = pNativeSpeaker->getAttrNameStr(
        L"Select a folder to add in Folder as Workspace panel",
        FOLDERASWORKSPACE_NODE, "SelectFolderFromBrowserString");
    wstring folderPath =
        folderBrowser(_pPublicInterface->getHSelf(), openWorkspaceStr);
    if (!folderPath.empty()) {
      if (_pFileBrowser ==
          nullptr) // first launch, check in params to open folders
      {
  ...
  case IDM_DOCLIST_FILESCLOSEOTHERS:
    if (_pDocumentListPanel) {
      ...
      if (id == IDM_DOCLIST_FILESCLOSEOTHERS) {
        ...
        _pDocumentListPanel->activateItem(_pEditView->getCurrentBufferID(),
                                          currentView());
      }
    }
    break;
  ...
  case IDM_DOCLIST_COPYPATHS:
    if (_pDocumentListPanel) {
      ...
      buf2Clipboard(buffers, id == IDM_DOCLIST_COPYPATHS,
  ...
  case IDM_FILE_CLOSEALL: {
    bool isSnapshotMode =
        NppParameters::getInstance().getNppGUI().isSnapshotMode();
  ...
  case IDM_EDIT_CUT: {
    ...
    if (focusedHwnd == _pEditView->getHSelf()) {
      ...
      } else if (NppParameters::getInstance()
                     .getSVP()
                     ._lineCopyCutWithoutSelection) // Cut the entire line with
                                                    // EOL
      {
  ...
  case IDM_EDIT_COPY: {
    ...
    if (focusedHwnd == _pEditView->getHSelf()) {
      if (_pEditView->hasSelection()) {
        ...
        _pEditView->execute(
            SCI_COPYALLOWLINE); // Copy without selected text, it will copy the
                                // whole line with EOL, for pasting before any
  ...
  case IDM_EDIT_CUT_BINARY: {
    ...
    HGLOBAL hglbCopy =
        ::GlobalAlloc(GMEM_MOVEABLE, (textLen + 1) * sizeof(unsigned char));
    if (!hglbCopy) {
      ::CloseClipboard();
      return;
    }
        ...
        if (IsClipboardFormatAvailable(cf_nppTextLen)) {
          ...
          if (hglbLen != NULL) {
            ...
            if (lpLen != NULL) {
              ...
              _pEditView->execute(SCI_ADDTEXT, *lpLen,
                                  reinterpret_cast<LPARAM>(lpchar));
  ...
  case IDM_EDIT_OPENASFILE: {
    if (_pEditView->execute(SCI_GETSELECTIONS) !=
        1) // Multi-Selection || Column mode || no selection
      return;
    ...
    ::SendMessage(hwnd, NPPM_GETFILENAMEATCURSOR, CURRENTWORD_MAXLENGTH,
                  reinterpret_cast<LPARAM>(currentWord.get()));

    wchar_t cmd2Exec[CURRENTWORD_MAXLENGTH] = {'\0'};
    if (id == IDM_EDIT_OPENINFOLDER) {
      wcscpy_s(cmd2Exec, L"explorer");
    } else {
      ::SendMessage(hwnd, NPPM_GETNPPFULLFILEPATH, CURRENTWORD_MAXLENGTH,
                    reinterpret_cast<LPARAM>(cmd2Exec));
    }
    ...
    if (doesPathExist(currentWord.get())) {
      ...
      if (id == IDM_EDIT_OPENINFOLDER ||
        ...
        ::ShellExecute(hwnd, L"open", cmd2Exec, fullFilePath.c_str(), L".",
                       SW_SHOW);
    } else // Relative file path - need concatenate with current full file path
    {
      ...
      ::SendMessage(hwnd, NPPM_GETCURRENTDIRECTORY, CURRENTWORD_MAXLENGTH,
                    reinterpret_cast<LPARAM>(currentDir.get()));
      ...
      if ((id == IDM_EDIT_OPENASFILE &&
           (!doesFileExist(fullFilePath.c_str() +
                           1)))) // + 1 for skipping the 1st char '"'
      {
        _nativeLangSpeaker.messageBox(
            "FilePathNotFoundWarning", _pPublicInterface->getHSelf(),
            L"The file you're trying to open doesn't exist.", L"File Open",
            MB_OK | MB_APPLMODAL);
        return;
      }
      // else id == IDM_EDIT_OPENINFOLDER - do it anyway. (even the last part
  ...
  case IDM_EDIT_SEARCHONINTERNET: {
    ...
    if (nppGui._searchEngineChoice == nppGui.se_custom) {
      ...
      if (url.empty() ||
          (httpPos != 0 &&
           httpsPos !=
               0)) // if string is not a url (for launching only browser)
      {
  ...
  case IDM_EDIT_PASTE_AS_HTML: {
    std::lock_guard<std::mutex> lock(command_mutex);
    UINT f = RegisterClipboardFormat(id == IDM_EDIT_PASTE_AS_HTML ? CF_HTML
                                                                  : CF_RTF);
  ...
  case IDM_EDIT_BEGINENDSELECT_COLUMNMODE: {
    ...
    ::CheckMenuItem(_mainMenuHandle, id,
                    MF_BYCOMMAND | (isStarted ? MF_CHECKED : MF_UNCHECKED));
    int otherId = (id == IDM_EDIT_BEGINENDSELECT)
                      ? IDM_EDIT_BEGINENDSELECT_COLUMNMODE
                      : IDM_EDIT_BEGINENDSELECT;
    ::EnableMenuItem(_mainMenuHandle, otherId,
                     MF_BYCOMMAND |
                         (isStarted ? (MF_DISABLED | MF_GRAYED) : MF_ENABLED));
  } break;
  ...
  case IDM_EDIT_SORTLINES_LENGTH_DESCENDING: {
    ...
    if (_pEditView->execute(SCI_GETSELECTIONS) > 1) {
      if (_pEditView->execute(SCI_SELECTIONISRECTANGLE) ||
        ...
        size_t rectSelAnchor =
            _pEditView->execute(SCI_GETRECTANGULARSELECTIONANCHOR);
        size_t rectSelCaret =
            _pEditView->execute(SCI_GETRECTANGULARSELECTIONCARET);
        size_t anchorLine =
            _pEditView->execute(SCI_LINEFROMPOSITION, rectSelAnchor);
        size_t caretLine =
            _pEditView->execute(SCI_LINEFROMPOSITION, rectSelCaret);
        fromLine = std::min<size_t>(anchorLine, caretLine);
        toLine = std::max<size_t>(anchorLine, caretLine);
        size_t anchorLineOffset =
            rectSelAnchor -
            _pEditView->execute(SCI_POSITIONFROMLINE, anchorLine) +
            _pEditView->execute(SCI_GETRECTANGULARSELECTIONANCHORVIRTUALSPACE);
        size_t caretLineOffset =
            rectSelCaret -
            _pEditView->execute(SCI_POSITIONFROMLINE, caretLine) +
            _pEditView->execute(SCI_GETRECTANGULARSELECTIONCARETVIRTUALSPACE);
      ...
      if (hasLineSelection) {
        const pair<size_t, size_t> lineRange =
            _pEditView->getSelectionLinesRange();
    ...
    bool isDescending =
        id == IDM_EDIT_SORTLINES_LEXICOGRAPHIC_DESCENDING ||
    ...
    if (id == IDM_EDIT_SORTLINES_LEXICOGRAPHIC_DESCENDING ||
        id == IDM_EDIT_SORTLINES_LEXICOGRAPHIC_ASCENDING) {
      pSorter = std::make_unique<LexicographicSorter>(isDescending, fromColumn,
                                                      toColumn);
    } else if (id == IDM_EDIT_SORTLINES_LEXICO_CASE_INSENS_DESCENDING ||
               id == IDM_EDIT_SORTLINES_LEXICO_CASE_INSENS_ASCENDING) {
      pSorter = std::make_unique<LexicographicCaseInsensitiveSorter>(
          isDescending, fromColumn, toColumn);
    } else if (id == IDM_EDIT_SORTLINES_INTEGER_DESCENDING ||
               id == IDM_EDIT_SORTLINES_INTEGER_ASCENDING) {
      pSorter =
          std::make_unique<IntegerSorter>(isDescending, fromColumn, toColumn);
    } else if (id == IDM_EDIT_SORTLINES_DECIMALCOMMA_DESCENDING ||
               id == IDM_EDIT_SORTLINES_DECIMALCOMMA_ASCENDING) {
      pSorter = std::make_unique<DecimalCommaSorter>(isDescending, fromColumn,
                                                     toColumn);
    } else if (id == IDM_EDIT_SORTLINES_DECIMALDOT_DESCENDING ||
               id == IDM_EDIT_SORTLINES_DECIMALDOT_ASCENDING) {
      pSorter = std::make_unique<DecimalDotSorter>(isDescending, fromColumn,
                                                   toColumn);
    } else if (id == IDM_EDIT_SORTLINES_LENGTH_ASCENDING ||
               id == IDM_EDIT_SORTLINES_LENGTH_DESCENDING) {
      pSorter = std::make_unique<LineLengthSorter>(isDescending, fromColumn,
                                                   toColumn);
    } else if (id == IDM_EDIT_SORTLINES_REVERSE_ORDER) {
      pSorter =
          std::make_unique<ReverseSorter>(isDescending, fromColumn, toColumn);
    } else {
      pSorter =
          std::make_unique<RandomSorter>(isDescending, fromColumn, toColumn);
    }
    try {
      ...
      _nativeLangSpeaker.messageBox(
          "SortingError", _pPublicInterface->getHSelf(),
          L"Unable to perform numeric sorting due to line $INT_REPLACE$.",
          L"Sorting Error", MB_OK | MB_ICONINFORMATION | MB_APPLMODAL,
          static_cast<int>(lineNo), 0);
    }
    _pEditView->execute(SCI_ENDUNDOACTION);
  ...
  case IDM_EDIT_SORTLINES_LOCALE_DESCENDING: {
    ...
    if (result.status)
      _nativeLangSpeaker.messageBox(
          result.tagName.data(), _pPublicInterface->getHSelf(),
          result.message.data(),
          result.status == MB_ICONERROR ? L"Sort Failed"
                                        : L"Sort not performed",
  ...
  case IDM_VIEW_PROJECT_PANEL_3: {
    ProjectPanel **pp[] = {&_pProjectPanel_1, &_pProjectPanel_2,
                           &_pProjectPanel_3};
  ...
  case IDM_VIEW_SWITCHTO_FILEBROWSER: {
        ...
        nullptr) // first launch, check in params to open folders
    {
      ...
      launchFileBrowser(nppParam.getFileBrowserRoots(),
                        nppParam.getFileBrowserSelectedItemPath());
      if (_pFileBrowser != nullptr) {
        checkMenuItem(IDM_VIEW_FILEBROWSER, true);
        _toolBar.setCheck(IDM_VIEW_FILEBROWSER, true);
        _pFileBrowser->setClosed(false);
  ...
  case IDM_VIEW_TAB_MOVEBACKWARD: {
    ...
    ::SendMessage(_pDocTab->getHSelf(), TCM_GETITEM, currentTabIndex,
                  reinterpret_cast<LPARAM>(&tciMove));

    ::SendMessage(_pDocTab->getHSelf(), TCM_GETITEM, newTabIndex,
                  reinterpret_cast<LPARAM>(&tciShift));
    ::SendMessage(_pDocTab->getHSelf(), TCM_SETITEM, currentTabIndex,
                  reinterpret_cast<LPARAM>(&tciShift));

    ::SendMessage(_pDocTab->getHSelf(), TCM_SETITEM, newTabIndex,
                  reinterpret_cast<LPARAM>(&tciMove));

    ::SendMessage(_pDocTab->getHSelf(), TCM_SETCURSEL, newTabIndex, 0);

    // Notify plugins that the document order has changed
    ::SendMessage(_pDocTab->getHParent(), NPPM_INTERNAL_DOCORDERCHANGED, 0,
                  newTabIndex);
  } break;
  ...
  case IDC_EDIT_TOGGLEMACRORECORDING: {
    if (_recordingMacro) {
      ...
      _mainEditView.execute(SCI_SETCURSOR,
                            static_cast<WPARAM>(SC_CURSORNORMAL));
  ...
  case IDM_MACRO_PLAYBACKRECORDEDMACRO:
    if (!_recordingMacro) // if we're not currently recording, then playback the
                          // recorded keystrokes
    {
  ...
  case IDM_EDIT_COPY_ALL_PATHS: {
    ...
    for (auto &&docTab : docTabs) {
      for (size_t i = 0, len = docTab->nbItem(); i < len; ++i) {
        ...
        if (docTabs.size() < 2 ||
            std::find(buffers.begin(), buffers.end(), buf) == buffers.end())
          buffers.push_back(buf);
      }
    }
    buf2Clipboard({buffers.begin(), buffers.end()},
                  id == IDM_EDIT_COPY_ALL_PATHS, _pPublicInterface->getHSelf());
  } break;
  ...
  case IDM_SEARCH_FINDINFILES: {
    ::SendMessage(_pPublicInterface->getHSelf(), NPPM_LAUNCHFINDINFILESDLG, 0,
                  0);
    break;
  }
  ...
  case IDM_SEARCH_FINDINCREMENT: {
    ...
    if (!str.empty()) // the selected text is not empty, then use it
      _incrementFindDlg.setSearchText(
          str.c_str(),
  ...
  case IDM_SEARCH_FINDPREV: {
    if (_findReplaceDlg.isCreated()) {
      ...
      if ((id == IDM_SEARCH_FINDPREV) && (op._searchType == FindRegex) &&
          !nppParams.regexBackward4PowerUser()) {
        ...
        if (status == FSEndReached) {
          wstring msg = _nativeLangSpeaker.getLocalizedStrFromID(
              "find-status-end-reached", FIND_STATUS_END_REACHED_TEXT);
          _findReplaceDlg.setStatusbarMessage(msg, FSEndReached);
        } else if (status == FSTopReached) {
          wstring msg = _nativeLangSpeaker.getLocalizedStrFromID(
              "find-status-top-reached", FIND_STATUS_TOP_REACHED_TEXT);
  ...
  case IDM_SEARCH_VOLATILE_FINDPREV: {
    ...
    op._searchType = FindNormal;
    op._whichDirection =
        (id == IDM_SEARCH_VOLATILE_FINDNEXT ? DIR_DOWN : DIR_UP);
  ...
  case IDM_SEARCH_MARKONEEXT5: {
    ...
    if (range.cpMax > range.cpMin) {
      ...
      _pEditView->execute(SCI_INDICATORFILLRANGE, range.cpMin,
  ...
  case IDM_EDIT_COLUMNMODETIP: {
    _nativeLangSpeaker.messageBox(
        "ColumnModeTip", _pPublicInterface->getHSelf(),
        ...
        L"Column Mode Tip", MB_OK | MB_APPLMODAL);
  } break;
  ...
  case IDM_EDIT_COLUMNMODE: {
    ...
    if (isFirstTime)
      _nativeLangSpeaker.changeDlgLang(_colEditorDlg.getHSelf(),
                                       "ColumnEditor");
  } break;
  ...
  case IDM_SEARCH_SELECTMATCHINGBRACES: {
    ...
    if (braceOpposite != -1) {
      if (id == IDM_SEARCH_GOTOMATCHINGBRACE)
        ...
        _pEditView->execute(SCI_SETSEL,
                            std::min<intptr_t>(braceAtCaret, braceOpposite),
                            std::max<intptr_t>(braceAtCaret, braceOpposite) +
                                1); // + 1 so we always include the ending brace
                                    // in the selection.

      // Update Scintilla's knowledge about what column the caret is in, so that
      // if user does up/down arrow as first navigation after the brace-match
      // operation, the caret doesn't jump to an unexpected column
  ...
  case IDM_LANG_USER_DLG: {
    ...
    if (!udd->isCreated()) {
      ...
      if ((isUDDlgDocked) && (isUDDlgVisible)) {
        ...
        auto *pWindow =
            bothActive() ? &_subSplitter : dynamic_cast<Window *>(_pDocTab);
  ...
  case IDM_EDIT_RMV_TAB: {
    ...
    size_t selEndLineNumber =
        _pEditView->execute(SCI_LINEFROMPOSITION, selEndPos);
    if ((nbSelections > 1) || (lineNumber != selEndLineNumber)) {
      // multiple-selection or multi-line selection; use Scintilla SCI_TAB /
      // SCI_BACKTAB behavior
      _pEditView->execute(forwards ? SCI_TAB : SCI_BACKTAB);
    } else {
      // zero-length selection (simple single caret) or selected text is all on
      // single line depart from Scintilla behavior and do it our way
      size_t currentIndent =
          _pEditView->execute(SCI_GETLINEINDENTATION, lineNumber);
      intptr_t indentDelta = _pEditView->execute(SCI_GETTABWIDTH);
      if (!forwards)
        indentDelta = -indentDelta;
      _pEditView->setLineIndent(
          lineNumber, static_cast<intptr_t>(currentIndent) + indentDelta);
    }
  } break;
  ...
  case IDM_EDIT_SPLIT_LINES: {
    if (_pEditView->execute(SCI_GETSELECTIONS) == 1) {
      ...
      auto anchorPos =
          _pEditView->execute(SCI_POSITIONFROMLINE, lineRange.first);
      auto caretPos =
          _pEditView->execute(SCI_GETLINEENDPOSITION, lineRange.second);
      ...
      if (edgeMode == EDGE_NONE) {
        ...
        auto textWidth = _pEditView->execute(SCI_TEXTWIDTH, STYLE_DEFAULT,
                                             reinterpret_cast<LPARAM>("P"));
        auto edgeCol = _pEditView->execute(
            SCI_GETEDGECOLUMN); // will work for edgeMode == EDGE_BACKGROUND
        if (edgeMode == EDGE_MULTILINE) {
          NppParameters &nppParam = NppParameters::getInstance();
          ScintillaViewParams &svp =
              const_cast<ScintillaViewParams &>(nppParam.getSVP());
          edgeCol = svp._edgeMultiColumnPos
                        .back(); // the LAST edge column specified by the user
        }
  ...
  case IDM_EDIT_TRIMALL: {
    ...
    doTrim(lineBoth);
    if (isEntireDoc || _pEditView->execute(SCI_GETANCHOR) !=
                           _pEditView->execute(SCI_GETCURRENTPOS))
      eol2ws();
    _pEditView->execute(SCI_ENDUNDOACTION);
    break;
  ...
  case IDM_EDIT_TOGGLESYSTEMREADONLY: {
    ...
    if (toggleReadOnlyFlagFromFileAttributes(buf->getFullPathName(),
                                             isSysReadOnly)) {
      ...
      if (_isAdministrator) {
        ...
        // Not in admin mode, and file might be protected. So we can't change
        // the file attributes.
        _nativeLangSpeaker.messageBox(
            "NoAdminRight2ChangeReadOnlyFileAttribute",
            _pPublicInterface->getHSelf(),
            L"Please run Notepad++ as administrator to change the file "
            L"attributes.",
            L"Changing file read-only attribute failed",
            MB_OK | MB_ICONWARNING);
      }
  ...
  case IDM_EDIT_MULTISELECTALLMATCHCASEWHOLEWORD: {
    _multiSelectFlag =
        id == IDM_EDIT_MULTISELECTALL
            ? 0
            : (id == IDM_EDIT_MULTISELECTALLMATCHCASE
                   ? SCFIND_MATCHCASE
                   : (id == IDM_EDIT_MULTISELECTALLWHOLEWORD
                          ? SCFIND_WHOLEWORD
                          : SCFIND_MATCHCASE | SCFIND_WHOLEWORD));

    // Don't use _pEditView->hasSelection() because when multi-selection is
    // active and main selection has no selection, it will cause an infinite
    // loop on SCI_MULTIPLESELECTADDEACH. See:
    // https://github.com/notepad-plus-plus/notepad-plus-plus/pull/14330#issuecomment-1797080251
    bool hasSelection = (_pEditView->execute(SCI_GETSELECTIONSTART) !=
                         _pEditView->execute(SCI_GETSELECTIONEND));
    if (!hasSelection)
  ...
  case IDM_EDIT_MULTISELECTNEXTMATCHCASEWHOLEWORD: {
    _multiSelectFlag =
        id == IDM_EDIT_MULTISELECTNEXT
            ? 0
            : (id == IDM_EDIT_MULTISELECTNEXTMATCHCASE
                   ? SCFIND_MATCHCASE
                   : (id == IDM_EDIT_MULTISELECTNEXTWHOLEWORD
                          ? SCFIND_WHOLEWORD
                          : SCFIND_MATCHCASE | SCFIND_WHOLEWORD));
  ...
  case IDM_EDIT_MULTISELECTSSKIP: {
    ...
    _pEditView->execute(
        SCI_SETSEARCHFLAGS,
        _multiSelectFlag); // use the last used flag to select the next one
    _pEditView->execute(SCI_MULTIPLESELECTADDNEXT);
  ...
  case IDM_VIEW_ALWAYSONTOP: {
    int check =
        (::GetMenuState(_mainMenuHandle, id, MF_BYCOMMAND) == MF_CHECKED)
            ? MF_UNCHECKED
            : MF_CHECKED;
    ::CheckMenuItem(_mainMenuHandle, id, MF_BYCOMMAND | check);
    SetWindowPos(_pPublicInterface->getHSelf(),
                 check == MF_CHECKED ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0,
  ...
  case IDM_VIEW_UNFOLD_CURRENT: {
    bool isToggleEnabled =
        NppParameters::getInstance().getNppGUI()._enableFoldCmdToggable;
  ...
  case IDM_VIEW_DISTRACTIONFREE: {
    if ((_beforeSpecialView._isDistractionFree &&
         _beforeSpecialView._isFullScreen && _beforeSpecialView._isPostIt) ||
  ...
  case IDM_VIEW_IN_IE: {
    ...
    if (!currentBuf->isUntitled()) {
      ...
      appEntry += appName;
      ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, appEntry.c_str(), 0, KEY_READ,
                     &hKey2Check);
      ::RegQueryValueEx(hKey2Check, L"", nullptr, &valType,
                        reinterpret_cast<LPBYTE>(valData), &valDataLen);
      ...
      if (hKey2Check && valData[0] != '\0') {
        ::ShellExecute(NULL, L"open", valData, fullCurrentPath.c_str(), NULL,
                       SW_SHOWNORMAL);
      } else if (id == IDM_VIEW_IN_EDGE) {
        ...
        ::ShellExecute(NULL, L"open",
                       L"shell:Appsfolder\\Microsoft.MicrosoftEdge_"
                       L"8wekyb3d8bbwe!MicrosoftEdge",
                       fullCurrentPath.c_str(), NULL, SW_SHOW);
      } else {
        _nativeLangSpeaker.messageBox(
            "ViewInBrowser", _pPublicInterface->getHSelf(),
            L"Application cannot be found in your system.",
            L"View Current File in Browser", MB_OK);
      }
  ...
  case IDM_VIEW_TAB_SPACE: {
    ...
    auto &svp1 = const_cast<ScintillaViewParams &>(
        NppParameters::getInstance().getSVP());
    svp1._whiteSpaceShow = isChecked;

    const bool allChecked = svp1._whiteSpaceShow && svp1._eolShow &&
                            svp1._npcShow && svp1._ccUniEolShow;
  ...
  case IDM_VIEW_NPC_CCUNIEOL: {
    ...
    for (auto pView : pViews) {
      ...
      if (pBuf->getLangType() == L_ERRORLIST) {
        pView->execute(SCI_STYLESETVISIBLE, static_cast<WPARAM>(SCE_ERR_ESCSEQ),
                       static_cast<LPARAM>(isChecked));
        pView->execute(SCI_STYLESETVISIBLE,
                       static_cast<WPARAM>(SCE_ERR_ESCSEQ_UNKNOWN),
                       static_cast<LPARAM>(isChecked));
      }
    }
  ...
  case IDM_VIEW_INDENT_GUIDE: {
    ...
    ScintillaViewParams &svp1 =
        (ScintillaViewParams &)(NppParameters::getInstance()).getSVP();
  ...
  case IDM_VIEW_WRAP: {
    ...
    // Therefore save view position before in IDM_VIEW_WRAP and restore after
    // SCN_PAINTED, as Scintilla-Doc. says
    if (!isWrapped) {
  ...
  case IDM_VIEW_SYNSCROLLH: {
    ...
    if (_syncInfo._isSynScrollH) {
      ...
      intptr_t pixel = _mainEditView.execute(SCI_TEXTWIDTH, STYLE_DEFAULT,
                                             reinterpret_cast<LPARAM>("P"));
  ...
  case IDM_VIEW_SUMMARY: {
    ...
    if (pNativeSpeaker) {
      ...
      if (fileLen != -1) {
        wstring filePathLabel = pNativeSpeaker->getLocalizedStrFromID(
            "summary-filepath", L"Full file path: ");
        wstring fileCreateTimeLabel = pNativeSpeaker->getLocalizedStrFromID(
            "summary-filecreatetime", L"Created: ");
        wstring fileModifyTimeLabel = pNativeSpeaker->getLocalizedStrFromID(
            "summary-filemodifytime", L"Modified: ");
      ...
      wstring nbCharLabel = pNativeSpeaker->getLocalizedStrFromID(
          "summary-nbchar", L"Characters (without line endings): ");
      wstring nbWordLabel =
          pNativeSpeaker->getLocalizedStrFromID("summary-nbword", L"Words: ");
      wstring nbLineLabel =
          pNativeSpeaker->getLocalizedStrFromID("summary-nbline", L"Lines: ");
      wstring nbByteLabel = pNativeSpeaker->getLocalizedStrFromID(
          "summary-nbbyte", L"Document length: ");
      wstring nbSelLabel1 = pNativeSpeaker->getLocalizedStrFromID(
          "summary-nbsel1", L" selected characters (");
      wstring nbSelLabel2 = pNativeSpeaker->getLocalizedStrFromID(
          "summary-nbsel2", L" bytes) in ");
      wstring nbRangeLabel =
          pNativeSpeaker->getLocalizedStrFromID("summary-nbrange", L" ranges");
      ...
      wstring summaryLabel =
          pNativeSpeaker->getLocalizedStrFromID("summary", L"Summary");

      ::MessageBox(_pPublicInterface->getHSelf(), characterNumber.c_str(),
                   summaryLabel.c_str(), MB_OK | MB_APPLMODAL);
    }
  } break;
  ...
  case IDM_VIEW_MONITORING: {
    ...
    if (curBuf->isMonitoringOn()) {
      ...
      if (doesFileExist(longFileName)) {
        if (curBuf->isDirty()) {
          _nativeLangSpeaker.messageBox(
              "DocTooDirtyToMonitor", _pPublicInterface->getHSelf(),
              L"The document is dirty. Please save the modification before "
              L"monitoring it.",
              L"Monitoring problem", MB_OK);
        } else {
  ...
  case IDM_FORMAT_TOMAC: {
    EolType newFormat = (id == IDM_FORMAT_TODOS)    ? EolType::windows
                        : (id == IDM_FORMAT_TOUNIX) ? EolType::unix
                                                    : EolType::macos;
    ...
    if (!buf->isReadOnly()) {
      ...
      _pEditView->execute(SCI_CONVERTEOLS,
                          static_cast<int>(buf->getEolFormat()));
    }
  ...
  case IDM_FORMAT_AS_UTF_8: {
    bool isUTF8System =
        NppParameters::getInstance().isCurrentSystemCodepageUTF8();
    ...
    if (originalEncoding != -1) {
      if (buf->isDirty()) {
        int answer = _nativeLangSpeaker.messageBox(
            "SaveCurrentModifWarning", _pPublicInterface->getHSelf(),
            L"You should save the current modification.\rAll the saved "
            L"modifications cannot be undone.\r\rContinue?",
            L"Save Current Modification", MB_YESNO);
      ...
      if (_pEditView->execute(SCI_CANUNDO) == TRUE) {
        int answer = _nativeLangSpeaker.messageBox(
            "LoseUndoAbilityWarning", _pPublicInterface->getHSelf(),
            L"You should save the current modification.\rAll the saved "
            L"modifications cannot be undone.\r\rContinue?",
            L"Lose Undo Ability Warning", MB_YESNO);
        if (answer == IDYES) {
          // Do nothing
        } else {
          return;
      ...
      if (um == uni8Bit) {
        ...
        _pEditView->execute(
            SCI_SETCODEPAGE,
            !nppParams.isCurrentSystemCodepageUTF8() ? CP_ACP : SC_CP_UTF8);
      } else
  ...
  case IDM_FORMAT_CONV2_UTF_16LE: {
    ...
    if (idEncoding != -1) {
      ...
      if (_pClipboardHistoryPanel)
        bPreviousCHPanelTrackingState =
            _pClipboardHistoryPanel->trackClipboardOps(
                false); // we do not want to track & show the next Clipboard op
      ...
      if (_pClipboardHistoryPanel)
        _pClipboardHistoryPanel->trackClipboardOps(
            bPreviousCHPanelTrackingState); // restore
      ...
      if (hglbClipboardCopy) {
        ...
        // no previous Clipboard data, clear the ones used by the Scintilla's
        // conversion
        if (::OpenClipboard(_pPublicInterface->getHSelf())) {
          ::EmptyClipboard();
          ::CloseClipboard();
  ...
  case IDM_SETTING_IMPORTPLUGIN: {
    ...
    if (copiedFiles.size()) {
      NativeLangSpeaker *pNativeSpeaker =
          (NppParameters::getInstance()).getNativeLangSpeaker();
      pNativeSpeaker->messageBox(
          "NeedToRestartToLoadPlugins", _pPublicInterface->getHSelf(),
          L"You have to restart Notepad++ to load plugins you installed.",
          L"Notepad++ needs to be relaunched", MB_OK | MB_APPLMODAL);
    }
    break;
  }
  ...
  case IDM_SETTING_IMPORTSTYLETHEMES: {
    ...
    vector<wstring> copiedFiles =
        addNppComponents(destDir, extFilterName, extFilter);
    for (size_t i = 0, len = copiedFiles.size(); i < len; ++i) {
      wstring themeName(
          themeSwitcher.getThemeFromXmlFileName(copiedFiles[i].c_str()));
  ...
  case IDM_SETTING_OPENPLUGINSDIR: {
    const wchar_t *pluginHomePath =
        NppParameters::getInstance().getPluginRootDir();
  ...
  case IDM_SETTING_SHORTCUT_MAPPER_RUN: {
    GridState st = id == IDM_SETTING_SHORTCUT_MAPPER_MACRO ? STATE_MACRO
                   : id == IDM_SETTING_SHORTCUT_MAPPER_RUN ? STATE_USER
                                                           : STATE_MENU;
    ShortcutMapper shortcutMapper;
    shortcutMapper.init(_pPublicInterface->getHinst(),
                        _pPublicInterface->getHSelf(), st);
  ...
  case IDM_SETTING_EDITCONTEXTMENU: {
    _nativeLangSpeaker.messageBox(
        "ContextMenuXmlEditWarning", _pPublicInterface->getHSelf(),
        L"Editing contextMenu.xml allows you to modify your Notepad++ popup "
        L"context menu on edit zone.\rYou have to restart your Notepad++ to "
        L"take effect after modifying contextMenu.xml.",
        L"Editing contextMenu", MB_OK | MB_APPLMODAL);
  ...
  case IDM_TOOL_MD5_GENERATEINTOCLIPBOARD: {
    if (_pEditView->execute(SCI_GETSELECTIONS) == 1) {
      ...
      if (strLen) {
        ...
        _pEditView->execute(SCI_GETSELTEXT, 0,
                            reinterpret_cast<LPARAM>(selectedStr));
  ...
  case IDM_TOOL_SHA1_GENERATE: {
    ...
    if (isFirstTime)
      _nativeLangSpeaker.changeDlgLang(_sha1FromTextDlg.getHSelf(),
                                       "SHA1FromTextDlg");
  } break;
  ...
  case IDM_TOOL_SHA1_GENERATEFROMFILE: {
    ...
    if (isFirstTime)
      _nativeLangSpeaker.changeDlgLang(_sha1FromFilesDlg.getHSelf(),
                                       "SHA1FromFilesDlg");
  } break;
  ...
  case IDM_TOOL_SHA512_GENERATE: {
    ...
    if (isFirstTime)
      _nativeLangSpeaker.changeDlgLang(_sha512FromTextDlg.getHSelf(),
                                       "SHA512FromTextDlg");
  } break;
  ...
  case IDM_TOOL_SHA512_GENERATEFROMFILE: {
    ...
    if (isFirstTime)
      _nativeLangSpeaker.changeDlgLang(_sha512FromFilesDlg.getHSelf(),
                                       "SHA512FromFilesDlg");
  } break;
  ...
  case IDM_TOOL_SHA512_GENERATEINTOCLIPBOARD: {
    if (_pEditView->execute(SCI_GETSELECTIONS) == 1) {
      ...
      if (strLen) {
        ...
        switch (id) {
        case IDM_TOOL_SHA1_GENERATEINTOCLIPBOARD: {
          calc_sha1(hash, reinterpret_cast<const uint8_t *>(selectedStr),
                    strlen(selectedStr));
          hashLen = hash_sha1;
        } break;

        case IDM_TOOL_SHA256_GENERATEINTOCLIPBOARD: {
          calc_sha_256(hash, reinterpret_cast<const uint8_t *>(selectedStr),
                       strlen(selectedStr));
          hashLen = hash_sha256;
        } break;

        case IDM_TOOL_SHA512_GENERATEINTOCLIPBOARD: {
          calc_sha_512(hash, reinterpret_cast<const uint8_t *>(selectedStr),
                       strlen(selectedStr));
          hashLen = hash_sha512;
        } break;
  ...
  case IDM_HOMESWEETHOME: {
    ::ShellExecute(NULL, L"open", L"https://notepad-plus-plus.org/", NULL, NULL,
                   SW_SHOWNORMAL);
    break;
  }
  ...
  case IDM_ONLINEDOCUMENT: {
    ::ShellExecute(NULL, L"open", L"https://npp-user-manual.org/", NULL, NULL,
                   SW_SHOWNORMAL);
    break;
  }
  ...
  case IDM_FORUM: {
    ::ShellExecute(NULL, L"open", L"https://community.notepad-plus-plus.org/",
                   NULL, NULL, SW_SHOWNORMAL);
    break;
  }
  ...
  case IDM_CONFUPDATERPROXY: {
    ...
    // downloads are secured with SSL on notepad_plus_plus.org
    const NppParameters &nppParams = NppParameters::getInstance();
    winVer ver = nppParams.getWinVersion();
    if (ver <= WV_XP) {
      long res = _nativeLangSpeaker.messageBox(
          "XpUpdaterProblem", _pPublicInterface->getHSelf(),
          L"Notepad++ updater is not compatible with XP due to the obsolete "
          L"security layer under XP.\rDo you want to go to Notepad++ page to "
          L"download the latest version?",
          L"Notepad++ Updater", MB_YESNO);

      if (res == IDYES) {
        ::ShellExecute(NULL, L"open",
                       L"https://notepad-plus-plus.org/downloads/", NULL, NULL,
                       SW_SHOWNORMAL);
      }
    } else {
      ...
      bool isCertifVerified =
          securityGuard.checkModule(updaterFullPath, nm_gup);
#endif
      if (isCertifVerified) {
        wstring param;
        if (id == IDM_CONFUPDATERPROXY) {
          if (!_isAdministrator) {
            _nativeLangSpeaker.messageBox(
                "GUpProxyConfNeedAdminMode", _pPublicInterface->getHSelf(),
                L"Please relaunch Notepad++ in Admin mode to configure proxy.",
                L"Proxy Settings", MB_OK | MB_APPLMODAL);
            return;
          }
          param = L"-options";
        } else {
  ...
  case IDM_LANG_OPENUDLDIR: {
    wstring userDefineLangFolderPath =
        NppParameters::getInstance().getUserDefineLangFolderPath();
    ::ShellExecute(_pPublicInterface->getHSelf(), L"open",
                   userDefineLangFolderPath.c_str(), NULL, NULL, SW_SHOW);
    break;
  }

  case IDM_LANG_UDLCOLLECTION_PROJECT_SITE: {
    ::ShellExecute(NULL, L"open",
                   L"https://github.com/notepad-plus-plus/userDefinedLanguages",
                   NULL, NULL, SW_SHOWNORMAL);
    break;
  }
  ...
  case IDM_WINDOW_WINDOWS: {
    ...
    _windowsDlg.init(_pPublicInterface->getHinst(),
                     _pPublicInterface->getHSelf(), _pDocTab);
    _windowsDlg.doDialog();
  } break;

  case IDM_WINDOW_SORT_FN_ASC: {
    WindowsDlg windowsDlg;
    windowsDlg.init(_pPublicInterface->getHinst(),
                    _pPublicInterface->getHSelf(), _pDocTab);
  ...
  case IDM_SYSTRAYPOPUP_NEWDOC: {
    ...
    ::ShowWindow(_pPublicInterface->getHSelf(),
                 nppGUI._isMaximized ? SW_MAXIMIZE : SW_SHOW);
  ...
  case IDM_SYSTRAYPOPUP_ACTIVATE: {
    ...
    // Send sizing info to make window fit (specially to show tool bar. Fixed
    // issue #2600)
    ::SendMessage(_pPublicInterface->getHSelf(), WM_SIZE, 0, 0);
  } break;
  ...
  case IDM_PINTAB: {
    ...
    ::SendMessage(_pPublicInterface->getHSelf(), WM_NOTIFY, 0,
                  reinterpret_cast<LPARAM>(&nmhdr));
    ::SendMessage(_pPublicInterface->getHSelf(), WM_SIZE, 0, 0);
  } break;

  default:
    if (id > IDM_FILEMENU_LASTONE &&
        id < (IDM_FILEMENU_LASTONE + _lastRecentFileList.getMaxNbLRF() + 1)) {
      ...
      ::GetMenuString(_mainMenuHandle, id, langName, menuItemStrLenMax,
                      MF_BYCOMMAND);
      ...
      int i = id - ID_MACRO;
      vector<MacroShortcut> &theMacros =
          (NppParameters::getInstance()).getMacroList();
    ...
    } else if (_pluginsManager.inDynamicRange(
                   id)) // in the dynamic range allocated with
                        // NPPM_ALLOCATECMDID
    {
    ...
    } else if ((id >= IDM_DROPLIST_MRU_FIRST) &&
               (id < (IDM_DROPLIST_MRU_FIRST +
                      static_cast<int32_t>(_pDocTab->nbItem())))) {
  ...
  if (_recordingMacro)
    ...
    // No need to record the following commands: all they do is execute
    // Scintilla commands, which are recorded instead.
    case IDM_EDIT_CUT:
    case IDM_EDIT_COPY:
    case IDM_EDIT_PASTE:
    case IDM_EDIT_LINE_UP:
    ...
    // The following 3 commands will insert date time string during the
    // recording: SCI_REPLACESEL will be recorded firstly, then SCI_ADDTEXT (for
    // adding date time string) So we erase these 2 unwanted commanded for
    // recording these 3 following commands.
    case IDM_EDIT_INSERT_DATETIME_SHORT:
    case IDM_EDIT_INSERT_DATETIME_LONG:
    case IDM_EDIT_INSERT_DATETIME_CUSTOMIZED: {
      ...
      if (lastIndex >= 2) {
        --lastIndex;
        if (_macro[lastIndex]._message == SCI_ADDTEXT &&
            _macro[lastIndex - 1]._message == SCI_REPLACESEL) {
