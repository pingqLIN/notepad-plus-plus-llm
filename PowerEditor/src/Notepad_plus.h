...
enum FileTransferMode { TransferClone = 0x01, TransferMove = 0x02 };
...
enum trimOp { lineHeader = 0, lineTail = 1, lineBoth = 2, lineEol = 3 };

enum spaceTab { tab2Space = 0, space2TabLeading = 1, space2TabAll = 2 };
...
struct QuoteParams {
  ...
  QuoteParams(const wchar_t *quoter, Speed speed, bool shouldBeTrolling,
              int encoding, LangType lang, const wchar_t *quote)
      : _quoter(quoter), _speed(speed), _shouldBeTrolling(shouldBeTrolling),
        _encoding(encoding), _lang(lang), _quote(quote) {}
...
public:
  ...
  // The following functions apply to a single buffer and don't need to worry
  // about views, with the exception of doClose, since closing one view doesn't
  ...
  BufferID doOpen(const std::wstring &fileName, bool isRecursive = false,
                  ...
                  const wchar_t *backupFileName = NULL,
  ...
  bool fileReload();
  bool
  fileClose(BufferID id = BUFFER_INVALID,
            int curView = -1); // use curView to override view to close from
  ...
  bool switchToFile(
      BufferID buffer); // find buffer in active view then in other view.
  //@}
  ...
  const wchar_t *fileSaveSession(size_t nbFile = 0,
  ...
  void macroPlayback(Macro macro,
                     std::vector<Document> *pDocs4EndUAIn = nullptr);
...
private:
  ...
  ScintillaEditView _subEditView = ScintillaEditView(
      MAIN_EDIT_ZONE); // only _mainEditView and _subEditView are MAIN_EDIT_ZONE
                       // comparing with other Scintilla controls
  ScintillaEditView _mainEditView = ScintillaEditView(
      MAIN_EDIT_ZONE); // only _mainEditView and _subEditView are MAIN_EDIT_ZONE
                       // comparing with other Scintilla controls
  ...
  // make sure we don't recursively call doClose when closing the last file with
  // -quitOnEmpty
  ...
  bool _isNppSessionSavedAtExit =
      false; // guard flag, it prevents emptying of the Notepad++ session.xml in
             // case of multiple WM_ENDSESSION or WM_CLOSE messages
  ...
  bool canHideView(
      int whichOne); // true if view can safely be hidden (no open docs etc)
  ...
  void
  loadBufferIntoView(BufferID id, int whichOne,
                     bool dontClose = false); // Doesn't _activate_ the buffer
  bool removeBufferFromView(
      BufferID id, int whichOne); // Activates alternative of possible, or
                                  // creates clean document if not clean already

  bool activateBuffer(
      BufferID id, int whichOne,
      bool forceApplyHilite = false); // activate buffer in that view if found
  ...
  void checkMenuItem(int itemID, bool willBeChecked) const {
    ::CheckMenuItem(_mainMenuHandle, itemID,
                    MF_BYCOMMAND | (willBeChecked ? MF_CHECKED : MF_UNCHECKED));
  }

  bool isConditionExprLine(intptr_t lineNumber);
  intptr_t findMachedBracePos(size_t startPos, size_t endPos, char targetSymbol,
                              char matchedSymbol);
  ...
  // void changeStyleCtrlsLang(HWND hDlg, int *idArray, const char
  // **translatedText);
  ...
  std::wstring exts2Filters(const std::wstring &exts, int maxExtsLen = -1)
      const; // maxExtsLen default value -1 makes no limit of whole exts length
  int setFileOpenSaveDlgFilters(
      CustomFileDialog &fDlg, bool showAllExt,
      int langType =
          -1); // showAllExt should be true if it's used for open file dialog -
               // all set exts should be used for filtering files
  Style *getStyleFromName(const wchar_t *styleName);
  bool dumpFiles(const wchar_t *outdir,
                 const wchar_t *fileprefix = L""); // helper func
  ...
  loadCommandlineParams(const wchar_t *commandLine,
                        const CmdLineParams *pCmdParams) {
    const CmdLineParamsDTO dto =
        CmdLineParamsDTO::FromCmdLineParams(*pCmdParams);
  ...
  static bool deleteForward(ScintillaEditView *pCurrentView,
  ...
  HMENU createMenuFromMenu(HMENU hSourceMenu,
  ...
  void loadPanelIcon(HINSTANCE hInst, DockingDlgInterface *panel,
                     HICON *phIcon);
